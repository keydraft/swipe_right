        ...
        const load = async () => {
             try {
                ...
                if (res.success) {
                    ...
                    // For now, let's assume the response has them or we filter all
                    const dcRes = await dcApi.getAll(0, 500, res.data.companyId, res.data.branchId);
                    if (dcRes.success) {
                        const linked = dcRes.data.content.filter(d => d.status === "INVOICED"); // Usually dc has invoiceId field but Response might not show it yet
                        // Filter logic depends on DC response having invoiceId. 
                        // Let's assume for now we filter by status or the backend returns them in detailed getById.
                        setDcs(linked);
                    }
                }
